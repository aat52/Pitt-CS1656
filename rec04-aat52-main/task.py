...
class Task(object):
    ...
    def t2(self):
        query = """
            ...
            FROM "{}"
            WHERE "inspect_dt" BETWEEN '{}' and '{}' AND "city" = '{}' AND "facility_name" LIKE 'Pitt%' """.format(self.resource_id, self.start_str, self.end_str, "Pittsburgh")
        response = requests.get(self.wprdc_api_endpoint, {'sql': query}, verify=False)
        df = pd.DataFrame.from_dict(json.loads(response.text)['result']['records'])
        return df
    ...
    def t3(self):
        query = """
            SELECT "facility_name" AS facility, "description_new" AS violation, "rating", "high", "medium", "low"
            FROM "{}"
            WHERE ("facility_name" LIKE '% Pitt' OR "facility_name" LIKE 'Pitt %' OR "facility_name" LIKE '% Pitt %')
            AND "inspect_dt" BETWEEN '{}' AND '{}'
            AND "city" = '{}' """.format(self.resource_id, self.start_str, self.end_str, "Pittsburgh", "V")
        response = requests.get(self.wprdc_api_endpoint, {'sql': query}, verify=False)
        df = pd.DataFrame.from_dict(json.loads(response.text)['result']['records'])
        return df
