...
class Task(object):
    ...
    def q1(self):
        query = '''
            SELECT grades.sid, year, semester, count(*)
            FROM courses NATURAL JOIN grades
            INNER JOIN students ON students.sid = grades.sid
            WHERE grade > 0
            GROUP BY grades.sid, year, semester
        '''
        self.cur.execute(query)
        all_rows = self.cur.fetchall()
        return all_rows
    ...
    def q4(self):
        query = '''
            SELECT firstName, lastName, courses.major, number
            FROM courses NATURAL JOIN grades
            INNER JOIN students ON students.sid = grades.sid
            INNER join majors ON majors.sid = grades.sid
            WHERE grade = 0 AND courses.major = majors.major
            ORDER BY firstName, lastName, courses.major, number
        '''
        self.cur.execute(query)
        all_rows = self.cur.fetchall()
        return all_rows
    ...
    def q5(self):
        query = '''
            SELECT professor, count(grade) AS success
            FROM courses NATURAL JOIN grades
            INNER JOIN students ON students.sid = grades.sid
            WHERE grade >=2
            GROUP BY professor
    ...
    def q6(self):
        query = '''
            SELECT courses.number as course_number, group_concat(firstName || ' ' || lastName, ', ') as name, AVG(grade) as avg_grade
            FROM students
            JOIN grades
            ON grades.sid = students.sid
            JOIN courses
            ON grades.cid = courses.cid
            WHERE grade >= 2
            GROUP BY courses.number
            HAVING avg_grade > 3
            ORDER BY avg_grade DESC, name, courses.cid
